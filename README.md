# BTC 分析师追踪与 LangGraph Agent 决策系统 📈

> 以 LangGraph Agent 为核心的 BTC 行情分析、分析师观点追踪、预测验证、虚拟交易与决策回放系统
...
**BTC 分析师追踪与 LangGraph Agent 决策系统**是一个面向 BTC 行情研判、分析师观点验证和智能交易决策的全栈应用。系统以本地数据库为事实来源，后端通过 FastAPI 提供 API 服务，前端通过 React + Vite 构建可视化面板，核心能力由 LangGraph Agent 工作流编排。

系统围绕“观点录入 → Agent 解析 → 预测拆分 → 行情跟踪 → 到期验证 → Agent 决策 → 虚拟交易 → 节点回放 → 报告生成”形成闭环。它不仅用于验证分析师观点质量，也用于观察预测改口行为、沉淀 Agent 决策依据、追踪每个节点的输入输出，并通过虚拟账户衡量策略表现。
...
| **Agent 决策中枢** | 以 LangGraph 编排多条 Agent 工作流，连接观点、行情、预测、交易和报告 |
| **观点结构化解析 Agent** | 优先调用 LLM 结构化解析，失败时自动回退规则版解析 |
| **改口检测** | 同一分析师同一周期在验证前方向反转时，标记旧预测并扣稳定性分 |
| **预测验证** | 根据到期行情进行规则验证，并可生成 LLM 解释与失败归因 |
| **交易决策 Agent** | 综合行情、指标、待验证预测和持仓状态，输出开多、开空或观望 |
| **虚拟交易执行** | 将 Agent 信号落地为虚拟交易，记录开仓、平仓、手续费、资金费、已实现/未实现盈亏 |
...
| 能力 | 说明 |
|------|------|
| **多工作流编排** | 使用 LangGraph 将观点解析、交易信号、预测验证和日报生成拆分为独立但可协同的工作流 |
| **结构化理解** | 将自然语言观点拆解为分析师、方向、目标价、周期、置信度和异常提示等结构化字段 |
| **市场上下文融合** | 决策时同时读取 BTC 行情、技术指标、待验证预测、历史表现和账户持仓 |
| **可解释交易信号** | 输出交易方向的同时保存信号理由、风险判断和节点执行明细 |
